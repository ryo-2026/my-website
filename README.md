...
スポーツ現場では「選手の体調やメンタル状態をコーチが把握しきれない」という課題があります。紙や口頭での管理では見落としが発生しやすく、特に体調の悪化を早期にキャッチすることが難しい状況でした。

この課題を解決するため、選手が毎日スマホから日報を送信し、コーチがリアルタイムで全選手の状態を把握できるWebアプリを開発しました。ネイティブアプリに近い体験を低コストで提供するためにPWAとして実装しています。

---

## 概要

選手が毎日のトレーニング内容・メンタル・体調を記録し、コーチがリアルタイムで状態を把握できるチーム管理アプリです。スマートフォンのホーム画面にインストールして使えるPWA（Progressive Web App）として動作します。
...
### コーチ向け
- 全選手の本日の日報状況を一覧表示・絞り込み（競技・学年・性別・パーソナル）
...
| セキュリティ | Web Crypto API（PINのハッシュ化）|
...
### ロールベースのアクセス制御
ユーザーのロールをデータベースで管理し、ロールに応じた画面を出し分けています。新規ユーザーはチームPINを入力することで選手として登録されます。

### PINのセキュアな保存
チーム参加PINは平文でなく、ブラウザ標準の暗号化APIを用いてハッシュ化した上でFirestoreに保存しています。平文PINはいかなるタイミングでも保存されません。
...
### プロフィールのロックフロー
選手が初回プロフィールを登録すると自動でロックされます。変更が必要な場合はコーチへ申請 → コーチが承認 → 1回だけ編集可能、という承認フローを実装することで、誤操作や不正な自己編集を防止しています。
